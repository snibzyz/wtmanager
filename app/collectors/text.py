"""Collector: extract imgtrans_*.json and *_source.txt."""
...
def collect_text(
    ...
    """
    Copy imgtrans_*.json and *_source.txt from episode subfolders.
    ...
    for sub in base.iterdir():
        ...
        for f in sub.iterdir():
            if not f.is_file():
                continue
            if f.name.startswith("imgtrans_") and f.name.endswith(".json"):
                to_copy.append((sub.name, f, f.name))
            elif f.name.endswith("_source.txt"):
                to_copy.append((sub.name, f, f.name))

    if not to_copy:
        _log("ไม่พบไฟล์ imgtrans_*.json หรือ *_source.txt")
        return False, log
    ...
    try:
        for ep_name, src, name in sorted(to_copy, key=lambda x: (x[0], x[2])):
            if per_episode:
                ...
                _log(f"✓ {name}")
        _log(f"ดึงไฟล์ถอดคำเสร็จ ({len(to_copy)} ไฟล์)")
        return True, log
